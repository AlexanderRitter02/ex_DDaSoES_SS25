...
class EQ(OP):
    ...
    def translate(self) -> str:
        ...
        for a in sorted(ands, key=lambda x: x.index):
            and_fuse_control_block_row += [
                "".join(
                    [
                        fuse
                        for _, row in sorted(
                            and_fuse_control[a.index].items(), reverse=True
                        )
                        for _, fuse in sorted(row.items(), reverse=True)
                    ]
                )
            ]
        ...
        for a in range(len(ands), 2 ** len(variables)):
            and_fuse_control_block_row += [
                "".join(
                    [
                        fuse
                        for _, row in sorted(and_fuse_control[a].items(), reverse=True)
                        for _, fuse in sorted(row.items(), reverse=True)
                    ]
                )
            ]
            ...
            for o in sorted(ors, key=lambda x: x.index):
                acc += [
                    "".join(
                        [
                            col[and_block_id]
                            for _, col in sorted(or_fuse_control[o.index].items())
                        ]
                    )
                ]
        ...
        op_ordering = ", ".join(
            str(v) for v in list(sorted(variables, key=lambda x: x.index, reverse=True))
        )

        # 12. Build output ordering
        out_ordering = ", ".join(
            [str(o) for o in sorted(ors, key=lambda o: o.index, reverse=True)]
        )
        ...
        translation += (
            f"  - Ordering of inputs (on wire): inputs[{len(variables) - 1} : 0] = '"
            + "{"
            + f"{op_ordering}"
            + "}\n"
        )
        translation += (
            f"  - Ordering of outputs (on wire): outputs[{len(ors) - 1} : 0] = '"
            + "{"
            + f"{out_ordering}"
            + "}\n"
        )
