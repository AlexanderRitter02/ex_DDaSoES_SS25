...
foreach f {display_package.sv status_module.sv moving_average.sv pipeline.sv tmp_sens.sv} {
  if { [string match *.vhd ${f}] } {
    read_hdl -language vhdl ${f} -define USE_CDC=0
  } elseif { [string match *.v ${f}] } {
    read_hdl -language v2001 ${f} -define USE_CDC=0
  } elseif { [string match *.svh ${f}] } {
    read_hdl -language sv ${f} -define USE_CDC=0
